...
int KgIngestionForce::execute(const Plan& p) {
    ...
    // Erwartung: Erster Op trägt Inputs:
    // 0: correlationId, 1: processName, 2: summary, 3: screenshotsJson (array)
    ...
    const std::string shotsJson = getStr(op.inputs, 3);
    ...
    try {
        ok = PythonWorker::instance().call([&]() -> bool {
            ...
            // ingestOccuredFailure(correlationId, processName, summary, screenshotsJson)
            // screenshotsJson: JSON-String (z. B. ["C:/.../shot1.png", "..."])
            py::object res = kgi.attr("ingestOccuredFailure")(
                py::str(corr), py::str(process), py::str(summary), py::str(shotsJson)
            );
            // optional: bool zurück von Python
